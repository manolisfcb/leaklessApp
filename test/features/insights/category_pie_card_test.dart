...
CategoryInsight _category(String id, {required int minor, required double share}) =>
    CategoryInsight(
      categoryId: id,
...
void main() {
  group('CategoryPieCard.topSlices', () {
    test('returns all categories with no Others bucket when <= 5', () {
      ...
      final slices = CategoryPieCard.topSlices(_insightsWith(categories), const {});

      expect(slices, hasLength(3));
      expect(slices.map((s) => s.categoryId), ['a', 'b', 'c']);
    });

    test('aggregates categories beyond the top 5 into a single Others bucket', () {
      final categories = [
        for (var i = 0; i < 7; i++)
      ...
      expect(
        slices.take(5).map((s) => s.categoryId),
